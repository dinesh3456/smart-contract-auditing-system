...
exports.StandardComplianceCheckerWithERC721 = exports.ERC721Checker = void 0;
const parser = __importStar(require("@solidity-parser/parser"));
const ERC20Checker_1 = require("./ERC20Checker");
class ERC721Checker {
    ...
    checkCompliance() {
        ...
        const requiredFunctions = [
            ...
            },
            {
                ...
                signature: "function safeTransferFrom(address from, address to, uint256 tokenId, bytes calldata data) external",
                purpose: "Safely transfers the ownership of a given token ID to another address with data parameter",
            },
            {
                name: "safeTransferFrom",
                signature: "function safeTransferFrom(address from, address to, uint256 tokenId) external",
                purpose: "Safely transfers the ownership of a given token ID to another address",
            },
            {
                name: "transferFrom",
                signature: "function transferFrom(address from, address to, uint256 tokenId) external",
                purpose: "Transfers the ownership of a given token ID to another address",
            },
            {
                name: "approve",
            ...
            },
            {
                ...
                signature: "function isApprovedForAll(address owner, address operator) external view returns (bool)",
        ...
        const requiredEvents = [
            ...
            },
            {
                ...
                purpose: "Emitted when owner enables or disables operator for all tokens",
            },
        ];
        ...
        const optionalFunctions = [
            ...
            {
                name: "tokenByIndex",
                signature: "function tokenByIndex(uint256 index) external view returns (uint256)",
                purpose: "Gets a token ID at a given index (ERC721Enumerable)",
            },
            {
                name: "tokenOfOwnerByIndex",
                signature: "function tokenOfOwnerByIndex(address owner, uint256 index) external view returns (uint256)",
                purpose: "Gets a token ID at a given index of the owned tokens list (ERC721Enumerable)",
            },
        ];
            ...
            ContractDefinition: (node) => {
                if (node.baseContracts) {
                    for (const base of node.baseContracts) {
                        if (base.baseName.namePath.includes("ERC721") ||
        ...
        const foundOptionalFunctions = this.findImplementedFunctions(optionalFunctions.map((f) => f.name));
        // Determine missing requirements
        const missingFunctions = requiredFunctions.filter((f) => !this.isFunctionImplemented(f.name, foundFunctions));
        ...
        if (missingFunctions.length > 0 || missingEvents.length > 0) {
            if (implementsERC721Interface) {
                recommendations.push("Your contract imports or inherits from ERC721, but does not implement all required functions or events.");
            }
            else {
                recommendations.push("Consider inheriting from the OpenZeppelin ERC721 implementation to ensure full compliance.");
            }
            if (missingEvents.length > 0) {
                recommendations.push("Always emit appropriate events (Transfer, Approval, ApprovalForAll) when state changes.");
            }
        }
        ...
        const hasMetadata = foundOptionalFunctions.includes("name") &&
            foundOptionalFunctions.includes("symbol") &&
            foundOptionalFunctions.includes("tokenURI");
        const hasEnumerable = foundOptionalFunctions.includes("totalSupply") &&
            foundOptionalFunctions.includes("tokenByIndex") &&
            foundOptionalFunctions.includes("tokenOfOwnerByIndex");
        if (!hasMetadata) {
            recommendations.push("Consider implementing ERC721Metadata interface (name, symbol, tokenURI) for better NFT compatibility.");
        }
        if (!hasEnumerable) {
            recommendations.push("Consider implementing ERC721Enumerable interface for better marketplace and indexer support.");
        }
     ...
     */
    isFunctionImplemented(functionName, foundFunctions) {
    ...
    checkSecurityIssues() {
        ...
        parser.visit(this.ast, {
            FunctionDefinition: (node) => {
                if (node.name === "safeTransferFrom") {
                    ...
                    if (node.modifiers &&
                        node.modifiers.some((m) => m.name.toLowerCase().includes("nonreentrant") ||
                    ...
                    parser.visit(node, {
                        ModifierInvocation: (modNode) => {
                            if (modNode.name.toLowerCase().includes("nonreentrant") ||
        ...
        if (!hasReentrancyProtection) {
            issues.push("Consider adding reentrancy protection to safeTransferFrom functions to prevent potential reentrancy attacks.");
        }
        // Check for zero address validation
        const hasZeroAddressCheck = this.hasZeroAddressValidation();
        if (!hasZeroAddressCheck) {
            issues.push("Consider adding checks to prevent transfers to the zero address (0x0) to avoid permanent loss of NFTs.");
        }
        ...
        if (!hasTokenExistenceCheck) {
            issues.push("Ensure tokenURI and ownerOf functions check that the tokenId exists before returning data.");
        }
        return issues;
    }
    /**
...
exports.ERC721Checker = ERC721Checker;
// Update the StandardComplianceChecker to include the ERC721 checker
class StandardComplianceCheckerWithERC721 {
    constructor(sourceCode) {
        this.sourceCode = sourceCode;
    }
    /**
     ...
     */
    checkERC20() {
        const checker = new ERC20Checker_1.ERC20Checker(this.sourceCode);
...
exports.StandardComplianceCheckerWithERC721 = StandardComplianceCheckerWithERC721;
